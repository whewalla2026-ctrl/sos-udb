...
import { GET_EVIDENCE_GALLERY, ADD_EVIDENCE } from '../../../lib/queries';
...
export default function EvidenceGalleryPage() {
  var { data: galleryData, loading } = useQuery(GET_EVIDENCE_GALLERY);
  var [addEvidence] = useMutation(ADD_EVIDENCE);
  var [showUpload, setShowUpload] = useState(false);
  var [gallery, setGallery] = useState(FALLBACK_GALLERY);
  var [uploadTitle, setUploadTitle] = useState('');

  useEffect(function() {
    if (galleryData?.evidenceGallery) {
      setGallery(galleryData.evidenceGallery);
    }
  }, [galleryData]);

  var handleUpload = function() {
    addEvidence({ variables: { title: uploadTitle || 'New Evidence', type: 'PHOTO', url: '' } }).catch(function(e) { console.error(e); });
    setShowUpload(false);
  };
  ...
  return (
    <div className="fade-in">
      <div className="page-header flex items-center justify-between">
        ...
        <button id="upload-evidence-btn" className="btn btn-primary" onClick={function() { setShowUpload(true); }}>+ Upload Proof</button>
      </div>

      {loading ? (
        <div className="glass-card" style={{ padding: 32, textAlign: 'center', color: 'var(--text-muted)' }}>Loading evidence gallery...</div>
      ) : (
        <div className="grid-cards">
          {gallery.map(function(item: any) {
            return (
              <div key={item.id} className="glass-card" style={{ overflow: 'hidden' }}>
                ...
                <div style={{ padding: 20 }}>
                  ...
                  {item.quest && <div style={{ color: 'var(--color-primary-light)', fontSize: '0.8125rem', fontWeight: 500, marginBottom: 12 }}>Quest: {item.quest}</div>}
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid var(--bg-glass-border)', paddingTop: 12, marginTop: 12 }}>
                    ...
                    <span style={{ color: 'var(--color-success)', fontSize: '0.8125rem', fontWeight: 600 }}>Approved</span>
                  </div>
                </div>
              </div>
            );
      ...
      {showUpload && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999 }}>
          <div className="glass-card scale-in" style={{ padding: 32, width: 400, textAlign: 'center' }}>
            ...
            <div style={{ border: '2px dashed var(--bg-glass-border)', borderRadius: 'var(--radius-md)', padding: 40, marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, textAlign: 'left' }}>
              <div>
                ...
                <input className="input mt-2" placeholder="What did you do?" value={uploadTitle} onChange={function(e) { setUploadTitle(e.target.value); }} />
              </div>
            </div>
            <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
              <button className="btn btn-secondary w-full" onClick={function() { setShowUpload(false); }}>Cancel</button>
              <button className="btn btn-primary w-full" onClick={handleUpload}>Upload</button>
            </div>
          </div>
        </div>
      )}
