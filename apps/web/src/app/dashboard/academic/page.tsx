...
import { GET_SKILL_GAPS } from '../../../lib/queries';
...
export default function AcademicPage() {
  var { data: gapsData, loading } = useQuery(GET_SKILL_GAPS);
  var [gaps, setGaps] = useState(FALLBACK_GAPS);

  useEffect(function() {
    if (gapsData?.mySkillGaps) {
      setGaps(gapsData.mySkillGaps);
    }
  }, [gapsData]);
  ...
  return (
    <div className="fade-in">
      <div className="page-header">
        ...
        <p className="page-subtitle">NWEA MAP scores and LMS integrations</p>
      </div>
      {loading ? (
        <div className="glass-card" style={{ padding: 32, textAlign: 'center', color: 'var(--text-muted)' }}>Loading skill gaps...</div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
          <div className="glass-card" style={{ padding: 24 }}>
            <h2 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: 20 }}>Skill Gap Analysis</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {gaps.map(function(g: any) {
                return (
                  <div key={g.subject}>
