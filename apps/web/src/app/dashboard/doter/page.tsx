    ...
    return (
      <div className="fade-in">
        <div className="page-header">
          ...
          <p className="page-subtitle">Your digital companion is loading...</p>
        </div>
      </div>
    );
  }

  var doter = meData?.me?.doterProfile || { name: 'Sparky', state: 'JUVENILE', level: 8, xp: 7200, streakDays: 12, isEnergetic: true, coinBalance: 0 };
  var xpNext = (doter.level + 1) * 1000;
  var xpPct = Math.round((doter.xp / xpNext) * 100);

  var stateEmoji: Record<string, string> = { EGG: '🥚', HATCHLING: '🐥', JUVENILE: '🐣', ADOLESCENT: '🦅', ADULT: '🦁', LEGENDARY: '🐉' };
  var evoStages = ['EGG', 'HATCHLING', 'JUVENILE', 'ADOLESCENT', 'ADULT', 'LEGENDARY'];
  var currentIdx = evoStages.indexOf(doter.state);
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '400px 1fr', gap: 24, alignItems: 'start' }}>
        <div className="glass-card" style={{ padding: 40, textAlign: 'center', background: 'var(--gradient-card)' }}>
          <div className="doter-container doter-state-energetic" style={{ marginBottom: 40, transform: 'scale(1.2)' }}>
            <div className="doter-orb" style={{ width: 160, height: 160, fontSize: '4rem' }}>{stateEmoji[doter.state] || '🐣'}</div>
          </div>
          ...
          <div style={{ textAlign: 'left', marginBottom: 24 }}>
            <div className="xp-bar-label">
              ...
              <span className="xp-bar-text">{doter.xp.toLocaleString()} / {xpNext.toLocaleString()}</span>
            </div>
            ...
            <div style={{ color: 'var(--text-muted)', fontSize: '0.8125rem', marginTop: 8, textAlign: 'center' }}>
              {xpNext - doter.xp > 0 ? `Only ${xpNext - doter.xp} XP to reach ${evoStages[Math.min(currentIdx + 1, evoStages.length - 1)]} state!` : 'Maximum level reached!'}
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div style={{ background: 'rgba(6,182,212,0.1)', border: '1px solid rgba(6,182,212,0.3)', padding: 12, borderRadius: 'var(--radius-md)' }}>
              <div style={{ fontSize: '1.5rem', marginBottom: 4 }}>⚡</div>
              <div style={{ fontWeight: 700, color: 'var(--color-accent)' }}>{doter.isEnergetic ? 'Energetic' : 'Normal'}</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          <div className="glass-card" style={{ padding: 24 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', position: 'relative' }}>
              ...
              <div style={{ position: 'absolute', top: 20, left: 20, width: `${(currentIdx / (evoStages.length - 1)) * 100}%`, height: 4, background: 'var(--gradient-primary)', zIndex: 1 }} />
              
              {evoStages.map(function(st, i) {
                var done = i < currentIdx;
                var active = i === currentIdx;
                var future = i > currentIdx;
                return (
                  <div key={st} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, zIndex: 2 }}>
                    <div style={{ 
                      width: 44, height: 44, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem',
                      background: active ? 'var(--gradient-primary)' : done ? 'var(--bg-elevated)' : 'var(--bg-base)',
                      border: `2px solid ${active ? 'var(--color-primary-light)' : done ? 'var(--color-primary)' : 'var(--bg-glass-border)'}`,
                      boxShadow: active ? 'var(--shadow-glow-primary)' : 'none',
                      opacity: future ? 0.5 : 1
                    }}>
                      {stateEmoji[st]}
                    </div>
                    <div style={{ fontSize: '0.6875rem', fontWeight: 600, color: active ? 'var(--color-primary-light)' : 'var(--text-muted)' }}>{st}</div>
                  </div>
                );
              })}
            </div>
          ...
          <div className="glass-card" style={{ padding: 24 }}>
            ...
            <div style={{ display: 'flex', gap: 12, alignItems: 'center', padding: 16, background: 'var(--bg-glass)', borderRadius: 'var(--radius-md)' }}>
              ...
              <div>
                <div style={{ fontWeight: 600, color: 'var(--color-accent)' }}>{doter.isEnergetic ? 'Energetic Buff Applied!' : 'No Active Buffs'}</div>
                <div style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
                  {doter.isEnergetic ? 'You completed activities today. All XP earned is boosted by 10%!' : 'Complete activities to get the Energetic buff.'}
                </div>
              </div>
            </div>
          </div>
