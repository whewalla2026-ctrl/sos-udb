...
var STEPS = ['Account', 'Family', 'Setup'];

export default function RegisterPage() {
  var [step, setStep] = useState(0);
  var [role, setRole] = useState<'PARENT' | 'CHILD'>('PARENT');
  var [form, setForm] = useState({ email: '', password: '', displayName: '', childName: '', childDob: '', consentMethod: 'CREDIT_CARD' });
  var [loading, setLoading] = useState(false);
  var [error, setError] = useState<string | null>(null);
  var { register } = useAuth();
  var router = useRouter();

  var next = () => setStep(s => Math.min(s + 1, 2));
  ...
  return (
    <div className="auth-layout">
      <div className="auth-card slide-up" style={{ maxWidth: 480 }}>
        {error && (
          <div style={{ background: 'rgba(239,68,68,0.15)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 'var(--radius-md)', padding: '12px 16px', marginBottom: 16, color: '#FCA5A5', fontSize: '0.875rem', display: 'flex', alignItems: 'center', gap: 8 }}>
            <span>⚠️</span>
            <span>{error}</span>
          </div>
        )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {[{ v: 'PARENT', icon: '👨‍👩‍👧', label: 'Parent / Guardian' }, { v: 'CHILD', icon: '🧒', label: 'Student (13+)' }].map(function(r) {
                  return (
            ...
            <div>
              ...
              <input className="input" placeholder="Your full name" value={form.displayName} onChange={function(e) { return setForm(function(f) { return { ...f, displayName: e.target.value }; }); }} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 500, color: 'var(--text-secondary)', marginBottom: 6 }}>Email</label>
              <input className="input" type="email" placeholder="you@example.com" value={form.email} onChange={function(e) { return setForm(function(f) { return { ...f, email: e.target.value }; }); }} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 500, color: 'var(--text-secondary)', marginBottom: 6 }}>Password</label>
              <input className="input" type="password" placeholder="Min 8 characters" value={form.password} onChange={function(e) { return setForm(function(f) { return { ...f, password: e.target.value }; }); }} />
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ background: 'rgba(124,58,237,0.1)', border: '1px solid rgba(124,58,237,0.3)', borderRadius: 'var(--radius-md)', padding: 16 }}>
              <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                🛡️ <strong style={{ color: 'var(--color-primary-light)' }}>COPPA Compliant.</strong> We require verified parental consent for children under 13.
              </p>
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 500, color: 'var(--text-secondary)', marginBottom: 6 }}>Child's Name</label>
              <input className="input" placeholder="Child's first name" value={form.childName} onChange={function(e) { return setForm(function(f) { return { ...f, childName: e.target.value }; }); }} />
            </div>
            <div>
              ...
              <input className="input" type="date" value={form.childDob} onChange={function(e) { return setForm(function(f) { return { ...f, childDob: e.target.value }; }); }} />
            </div>
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {[{ v: 'CREDIT_CARD', label: '💳 Credit Card Verification', desc: 'A $0.01 hold placed and released' }, { v: 'ID_CHECK', label: '🪪 Government ID Upload', desc: 'Verified by our compliance team within 24h' }].map(function(m) {
                  return (
                    <button key={m.v} id={'consent-' + m.v.toLowerCase()} onClick={function() { return setForm(function(f) { return { ...f, consentMethod: m.v }; }); }}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div style={{ textAlign: 'center', padding: '20px 0' }}>
              ...
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.9375rem', lineHeight: 1.6 }}>
                Your Doter starts as an egg and evolves as {form.childName || 'your child'} completes quests, learns, and grows.
              </p>
            </div>
            ...
            <div style={{ background: 'var(--bg-glass)', border: '1px solid var(--bg-glass-border)', borderRadius: 'var(--radius-md)', padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[['Account', form.displayName, '✓'], ['Email', form.email, '✓'], ['Child', form.childName || 'Not set', form.childName ? '✓' : '⚠'], ['Consent', form.consentMethod.replace('_', ' '), '✓']].map(function(item) {
                var k = item[0], v = item[1], s = item[2];
                return (
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <button className="btn btn-secondary" style={{ flex: 1 }} onClick={back} disabled={loading}>← Back</button>
