...
export function ParentDashboard({ data }: { data: any }) {
  ...
  var displayName = user?.displayName || 'Parent';
          ...
          <div>
            ...
            {!children || children.length === 0 ? (
              <div className="glass-card" style={{ padding: 32, textAlign: 'center', color: 'var(--text-muted)' }}>
                No children linked yet. Add a family member to get started.
              </div>
            ) : (
                ...
                {children.map((child: any) => (
                  <div key={child.id} className="glass-card" style={{ padding: 20 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16 }}>
                      {child.avatarUrl && <img src={child.avatarUrl} alt={child.name} style={{ width: 60, height: 60, borderRadius: '50%' }} />}
                      <div>
                        <h3 style={{ fontSize: '1.125rem', fontWeight: 600 }}>{child.name || child.displayName || 'Child'}</h3>
                        <div style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>Doter: {child.doterName || 'Unnamed'} (Lv {child.doterLevel || 1})</div>
                      </div>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                      ...
                      <div className="stat-card" style={{ padding: '12px 16px' }}>
                        ...
                        <div style={{ fontWeight: 600, color: '#F59E0B' }}>{child.points || 0}</div>
                      </div>
                    </div>
                ...
                {pendingApprovals.map((item: any) => (
                  <div key={item.id} className="glass-card" style={{ padding: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      ...
                      <button className="btn btn-primary" style={{ padding: '6px 12px', fontSize: '0.8125rem' }}>Approve (+{item.xpReward || 0} XP)</button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          <div className="glass-card" style={{ padding: 24 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <a href="/dashboard/quests" className="btn btn-outline" style={{ display: 'flex', justifyContent: 'flex-start', padding: 12, textDecoration: 'none' }}>
                Assign Custom Quest
              </a>
              <a href="/dashboard/ventures" className="btn btn-outline" style={{ display: 'flex', justifyContent: 'flex-start', padding: 12, textDecoration: 'none' }}>
                Review Escrow Holds
              </a>
          ...
          <div className="glass-card" style={{ padding: 24, background: 'rgba(236,72,153,0.05)', border: '1px solid rgba(236,72,153,0.2)' }}>
            ...
            <a href="/dashboard/admin/audit" style={{ color: '#EC4899', fontSize: '0.875rem', fontWeight: 500, textDecoration: 'none' }}>
